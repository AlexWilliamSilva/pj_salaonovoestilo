<?php
    include_once "header.php";
    include_once "nav.php";
    include_once "footer.php";
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Salão Novo Estilo</title>
        <link rel="stylesheet" href="style/styleservicos.css">
</head>
<body>
        <main>
                <h1>Serviços</h1>

           <div id="AreaMasculino">
            
                <h3>Serviços na área Masculino</h3>

                    <!-- serviço 01 -->
                    <div id="ProgressivaMasculino">
                            <h3>Progressiva Masculino</h3>
                                <p>Alisamento e tratamento para cabelos masculinos.</p>
                    </div> 
                       <!-- fecha serviço 01 -->
                        
                       <!-- serviço 02 -->
                    <div id="CortesdeCabeloMasculino">
                            <h3>Cortes de Cabelo Masculino</h3>
                                <p>Cortes clássicos e modernos para atender ao seu estilo e personalidade.</p>
                    </div>    
                        <!-- fecha serviço 02 -->

                        <!-- serviço 03 -->
                    <div id="LuzesMasculino">
                            <h3>Luzes Masculino</h3>
                                <p>Serviço de luzes para realçar o visual dos cabelos masculinos.</p>
                    </div>
                         <!-- fecha serviço 03 --> 
                          
                          <!-- serviço 04 -->                       
                    <div id="Sobrancelhamasculina">
                            <h3>Sobrancelha masculina</h3>
                                <p>Design e cuidados para manter a sobrancelha masculina alinhada.</p>
                    </div> 
                        <!-- fecha serviço 04 -->
                         
                        <!-- serviço 05 -->    
                    <div id="RelaxamentoCapilarmasculino">
                            <h3>Relaxamento Capilar masculino</h3>
                                <p>Tratamento para relaxar os fios e facilitar o penteado.</p>
                    </div> 
                        <!-- fecha serviço 05 --> 
                         
                        <!-- serviço 06 -->
                    <div id="Barba">
                            <h3>Barba</h3>
                                <p>Modelagem, aparo e cuidados completos para a barba.</p>
                    </div> 
                        <!-- fecha serviço 06 --> 
                      
                        <!-- serviço 07 --> 
                    <div id="LimpezadePele">
                            <h3>Limpeza de Pele</h3>
                                <p>Tratamentos faciais para limpeza profunda e revitalização da pele.</p>
                    </div>  
                         <!-- fecha serviço 07 -->       
           </div>
        
           
        <!-- Area feminida  -->
           <div id="AreaFeminina">

                <h3>Serviços na área Feminina</h3>

                        <!-- serviço 01  -->
                    <div id="CortesdecabeloFeminino">
                            <h3>Cortes de cabelo Feminino</h3>
                                <p>Estilos variados para realçar a beleza e elegância feminina.</p>
                    </div>  
                        <!-- fecha serviço 01  -->

                        <!-- serviço 02  -->
                    <div id="ProgressivaFeminino">
                            <h3>Progressiva Feminino</h3>
                                <p>Alisamento e tratamento para cabelos femininos.</p>
                    </div>  
                        <!-- fecha serviço 02  -->

                        <!--  serviço 03  -->
                    <div id="LimpezadePele">
                            <h3>Limpeza de Pele</h3>
                                <p>Tratamentos faciais para limpeza profunda e revitalização da pele.</p>
                    </div>  
                         <!--  fecha serviço 03  -->
           </div>

        </main>
    
</body>
</html>